...
const StoriesPublished = ({stories}: Props) => {
   const {user} = useAuth()
   ...
   return (
      <article className='flex flex-1 flex-col gap-10 px-3'>
         <Head>
            <title>Your stories - Published</title>
...
export const getServerSideProps = async (ctx: GetServerSidePropsContext) => {
   ...
   try {
      ...
      const {name} = token
      const qDrafts = query(collection(db, `users/${name}/stories`), where('status', '==', 'published'), orderBy('timeStamp', "desc"))
...
export default StoriesPublished